 ...
 */
const long  LIS2DS12_Sensitivity_List[3][4] = {
    /* HR 14bit */
    {
       244,  /* FS @2g */
      1952,  /* FS @16g */
...
status_t LIS2DS12_Get_Acceleration(int *buff)
{
  ...
  odr = (ctrl1_reg >> 4) & 0xF;//CTRL1(ODR[3:0])
  hf  = (ctrl1_reg >> 1) & 0x1;//CTRL1(HF_ODR)
  fs  = (ctrl1_reg >> 2) & 0x3;//CTRL1(FS[1:0])

  /* Determine which of the 3 cases the device is in */
  if (odr >= 8) {
    /* LP_10 case */
    shift = 6;//bit resolution is 10
    mode = 2;
  } else if (odr > 4 && odr < 8 && hf == 1) {
    /* HF_12 case */
    shift = 4;//bit resolution is 12
    mode = 1;
  } else {
    /* HR_14 case */
    shift = 2;// bit resolution is 14
    mode = 0;
  }
...
status_t LIS2DS12_Get_6D_Event(u8_t *val){
  ...
  case LIS2DS12_XL_EV_ON:
    *val = LIS2DS12_XL_EV_ON;//1, for up-right side position
    break;
  case LIS2DS12_XH_EV_ON:
    *val = LIS2DS12_XH_EV_ON;//2, for down-left side position
    break;
  case LIS2DS12_YL_EV_ON :
     *val = LIS2DS12_YL_EV_ON;//4, for up-left side position
     break;
  case LIS2DS12_YH_EV_ON:
    *val = LIS2DS12_YH_EV_ON;//8, for down-right side position
    break;
  case LIS2DS12_ZH_EV_ON:
    *val = LIS2DS12_ZH_EV_ON;//32, up face
    break;
  case LIS2DS12_ZL_EV_ON:
    *val = LIS2DS12_ZL_EV_ON;//16, down face
    break;
  }
  return MEMS_SUCCESS;
}
